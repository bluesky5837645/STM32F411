...
//float beta = sqrt(3.0f / 4.0f) * PI * (40.0f / 180.0f);   // compute beta
//float zeta = sqrt(3.0f / 4.0f) * PI * (0.0f  / 180.0f);   // compute zeta, the other free parameter in the Madgwick scheme usually set to a small or zero value

float beta = 0.9045998;
float zeta = 0.0;
...
uint8_t MPU9255_Init(I2C_HandleTypeDef *I2Cx){
	...
	memset(data, '\0', sizeof(data));
	sprintf(data, "MPU9250 STM32 Implementation \r\n");
	HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
	...
	if (SerialDebugA) {
		memset(data, '\0', sizeof(data));
		sprintf(data, "MPU -WHO AM I- is: (Must return 113) %d\r\n", readData);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
//		printf("MPU -WHO AM I- is: (Must return 113) %d\r\n", readData);
	...
	if (readData == 113) {
		...
		if (SerialDebugA) {
			memset(data, '\0', sizeof(data));
			sprintf(data, "MAG -WHO AM I- is: (Must return 72) %d\r\n", readData);
			HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
...
void readAll(I2C_HandleTypeDef *I2Cx, MPU9255_t*DataStruct) {
	...
	pitch = asinf(2.0 * (q[0] * q[2] - q[1] * q[3]));
	roll  = atan2f((q[0] * q[1] + q[2] * q[3]), 0.5 - (q[1] * q[1] + q[2] * q[2]));
	yaw   = atan2f((q[1] * q[2] + q[0] * q[3]), 0.5 - ( q[2] * q[2] + q[3] * q[3]));
	pitch *= 180.0f / PI;
	yaw   *= 180.0f / PI;
	yaw = -yaw;
	yaw   += 15.0f; // Declination
...
void readAccelData(I2C_HandleTypeDef *I2Cx, int16_t * destination){
  ...
  if(SerialDebugB){
		memset(data, '\0', sizeof(data));
		sprintf(data, "Acc X: %d\r\n",destination[0]);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Acc Y: %d\r\n",destination[1]);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Acc Z: %d\r\n",destination[2]);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
...
void readGyroData(I2C_HandleTypeDef *I2Cx, int16_t * destination){
  ...
  if(SerialDebugB){
		memset(data, '\0', sizeof(data));
		sprintf(data, "Gyro X: %d\r\n",destination[0]);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Gyro Y: %d\r\n",destination[1]);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Gyro Z: %d\r\n",destination[2]);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
...
void readMagData(I2C_HandleTypeDef *I2Cx, int16_t * destination){
	...
	if( (readData & 0x01) == 0x01 ){
		...
		if(!(c & 0x08)) {
			...
			if(SerialDebugB){
				memset(data, '\0', sizeof(data));
				sprintf(data, "Mag X: %d\r\n",destination[0]);
				HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

				memset(data, '\0', sizeof(data));
				sprintf(data, "Mag Y: %d\r\n",destination[1]);
				HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

				memset(data, '\0', sizeof(data));
				sprintf(data, "Mag Z: %d\r\n",destination[2]);
				HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
...
void initAK8963(I2C_HandleTypeDef *I2Cx, float * destination){
  ...
  if(SerialDebugA){
		memset(data, '\0', sizeof(data));
		sprintf(data, "Mag cal off X: %d\r\n",destination[0]);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Mag cal off Y: %d\r\n",destination[1]);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Mag cal off Z: %d\r\n",destination[2]);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
...
//HAL_I2C_Mem_Write(I2Cx, AK8963_ADDRESS, AK8963_CNTL, 1, &writeData, 1, i2c_timeout);
  ...
  if(SerialDebugA){
		memset(data, '\0', sizeof(data));
		sprintf(data, "MAG Init Succesful! \r\n");
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
...
void calibrateMag(I2C_HandleTypeDef *I2Cx, float * dest1, float * dest2){
  ...
  if (SerialDebugA){
		memset(data, '\0', sizeof(data));
		sprintf(data, "Mag Calibration: Wave device in a figure eight until done!\r\n");
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
//	  printf("Mag Calibration: Wave device in a figure eight until done!\r\n");
  ...
  HAL_Delay(4000);
   ...
   for(ii = 0; ii < sample_count; ii++) {
    ...
    if (SerialDebugA){
		memset(data, '\0', sizeof(data));
		sprintf(data, "Mag Calibration done!\r\n");
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
...
void initMPU9250(I2C_HandleTypeDef *I2Cx){
	...
	if(SerialDebugA){
		memset(data, '\0', sizeof(data));
		sprintf(data, "MPU Init Succesful! \r\n");
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
...
void calibrateMPU9250(I2C_HandleTypeDef *I2Cx, float * dest1, float * dest2){
  ...
  uint8_t writeData;
	...
	if(SerialDebugA){
		...
		memset(data, '\0', sizeof(data));
		sprintf(data, "Gyro bias X: %f\r\n", gyroBiasX);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Gyro bias Y: %f\r\n", gyroBiasY);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Gyro bias Z: %f\r\n", gyroBiasZ);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
...
//		printf("-------------------------\r\n");
	...
	if(SerialDebugA){
		...
		memset(data, '\0', sizeof(data));
		sprintf(data, "Accel bias X: %f\r\n", accelBiasX);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Accel bias Y: %f\r\n", accelBiasY);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Accel bias Z: %f\r\n", accelBiasZ);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
...
void MPU9250SelfTest(I2C_HandleTypeDef *I2Cx, float * destination) {
	...
	if(SerialDebugA){
		...
		memset(data, '\0', sizeof(data));
		sprintf(data, "Accel Test X: %f\r\n", testResultAccelX);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Accel Test Y: %f\r\n", testResultAccelY);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Accel Test Z: %f\r\n", testResultAccelZ);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Gyro Test X: %f\r\n", testResultGyroX);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Gyro Test Y: %f\r\n", testResultGyroY);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);

		memset(data, '\0', sizeof(data));
		sprintf(data, "Gyro Test Z: %f\r\n", testResultGyroZ);
		HAL_UART_Transmit(&huart2, (uint8_t *)data, strlen(data), 0xffff);
